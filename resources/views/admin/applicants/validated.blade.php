...
<div class="container mt-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="text-nche-primary fw-bold mb-0">Recognised Applications</h2>
            <div class="underline-accent mt-1"></div>
        </div>
        <span class="badge bg-nche-secondary fs-6 px-3 py-2">
            Total: {{ $users->sum(fn($u) => $u->applications->where('status', 'validated')->count()) }}
        </span>
    </div>

    @php
        // Filter only non-admin users with validated (recognised) applications
        $filteredUsers = $users->filter(function ($user) {
            return $user->applications->where('status', 'validated')->count() > 0 && !$user->hasRole('admin');
        });
    @endphp
            ...
            <table class="table table-bordered table-striped table-hover align-middle mb-0">
                ...
                <tbody>
                    @foreach($filteredUsers as $user)
                        @foreach($user->applications->where('status', 'validated') as $app)
                            <tr>
    ...
    @else
        <div class="alert alert-info mt-4 shadow-sm border-0 rounded-3">
            No recognised applications found.
        </div>
    @endif
</div>
